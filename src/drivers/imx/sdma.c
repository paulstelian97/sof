...
struct sdma_pdata {
	/* Statically allocate channel private data and contexts array. CCBs must be
	 * allocated as array anyway.
	 */
...
{
	trace_sdma_error("sdma_set_overrides(%d, %d)", event_override,
		host_override);
...
static void sdma_set_event(struct dma_chan_data *channel, int eventnum)
{
	...
	/* Set correct overrides for event-driven channels: EVTOVR=0 HOSTOVR=1 */
...
static int sdma_set_config(struct dma_chan_data *channel, struct dma_sg_config *config) {
	...
	if (config->cyclic)
		...
		pdata->descriptors[config->elem_array.count - 1].config &= ~SDMA_BD_CONT;
